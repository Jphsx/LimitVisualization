...
for page in xrange(n):
   ...
   # if args.checkboxes:
        ...
        #pads = plot.MultiRatioSplitColumns([0.54],[0.,0.])
        #pads[2].SetGrid(1, 0)
	#pads[1].SetGrid(1,0)
    if True:
        pads = plot.MultiRatioSplitColumns([0.95], [0.],[0.])
    pads[0].SetGrid(1, 0)
    pads[0].SetTickx(1)
    pads[1].SetGrid(1, 0)
    pads[1].SetTickx(1)
    ...
    #Set range
    centralVal=pdata[0]['fit'][1]
    h_pulls = ROOT.TH2F("pulls", "pulls", 6, centralVal-1.0, centralVal+1.0, n_params, 0, n_params)
    ...
    for p in xrange(n_params):
		...
		pull_hi= fit[2]- fit[1]
            ...
            #g_pulls.SetPoint(i,pull,float(i))
    ...
    if True:
        plot.Set(h_pulls.GetXaxis(), TitleSize=0.04, LabelSize=0.03, Title='Expected 50%: r < x (#pm #sigma)')
    ...
    # Go to the other pad and draw the impacts histo #skip drawing impacts
    '''
    pads[1].cd()
